...
Проанализированы четыре сборки: gcc -O0, gcc -O3, clang -O0, clang -O3. В каждом случае исследовались функции ScenarioA, ScenarioB и SigfpeHandler.
...
Наиболее агрессивная оптимизация. Функции полностью встроены в main. Для ScenarioB применена оптимизация на основе Undefined Behavior:
...
idiv %ecx
cmpl $0x0,0x4(%rsp)
Ветвление if (b == 0) полностью исчезло! Вместо проверки делителя сразу выводится "b is NOT 0" (или "b is 0" в зависимости от результата), потому что компилятор знает: деление на ноль — это UB, и если программа дошла до этой точки, делитель не мог быть нулём.

SigfpeHandler ещё компактнее: проверки байтов объединены, используется cmpb и je, обнуление через xorps и movups. Ветвление if (b == 0) в ScenarioB отсутствует.
...
При clang -O3 компилятор вывел из факта выполнения деления, что условие всегда выполняется.
...
Единственный надёжный способ избежать деления на ноль — проверять делитель до операции деления, не полагаясь на перехват аппаратного исключения.
